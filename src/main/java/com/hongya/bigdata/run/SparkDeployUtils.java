package com.hongya.bigdata.run;

import com.github.fommil.netlib.BLAS;
import com.google.common.collect.Sets;
import com.hongya.bigdata.algorithm.MonitorThread;
import com.hongya.bigdata.datastructure.FixSizePriorityQueue;
import com.hongya.bigdata.model.MIdRated;
import com.hongya.bigdata.model.Movie;
...
public class SparkDeployUtils {
	...
	static{
		...
		try {
			// input = new FileInputStream("hadoop.properties");
            input = SparkDeployUtils.class.getClassLoader().getResourceAsStream("hadoop.properties");
			prop.load(input);
	...
	public static String runSpark(String[] args) {
	    ...
	    if (System.getProperty("spark.master","local[2]").contains("local")){
		...
		System.out.println("runSpark args:"+buff.toString());
		try {
...
//			ClientArguments cArgs = new SparkSubmitArguments(new scala.collection.mutable.ArrayBuffer<String>(args));
			...
			System.out.println(allAppStatus.toString());
	...
	public static void cleanupStagingDir(ApplicationId appId) {
		...
		try {
			...
			if (fs.exists(stagingDirPath)) {
				System.out.println("Deleting staging directory " + stagingDirPath);
				fs.delete(stagingDirPath, true);
			}
		} catch (IOException e) {
			System.err.println("Failed to cleanup staging dir " + appStagingDir + e);
		}
	}
	...
	public static List<Movie> predict(int uid, int recNum) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException {
		...
		Set<Integer> candidates = Sets.difference(allMovieIds, userWithRatedMovies.get(uid).stream().map(MIdRated::getMovieId).collect(Collectors.toSet()));
